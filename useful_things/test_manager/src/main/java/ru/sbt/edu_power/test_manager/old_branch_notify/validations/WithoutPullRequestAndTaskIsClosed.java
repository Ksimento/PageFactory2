...
public class WithoutPullRequestAndTaskIsClosed {
    ...
    private boolean taskIsClosed(final Branch branch) {
        if (branch.getBranchModel().getMetadata().getBranchListJiraIssuesModel().isEmpty()) {
            log.info(
                    "Не удалось получить branch-list-jira-issues:\n{}",
                    new GsonBuilder().setPrettyPrinting().create().toJson(branch.getBranchModel().getMetadata())
            );
            return true;
        }
        ...
        try {
            ...
            throw new TestManagerException(e);
        }
    }
    ...
    private void sendMessageToChannelIfTaskIsClosed(final List<Branch> branchList) {
        ...
        sendPersonalMessage(
                branchList,
                String.format("Вам необходимо удалить указанные ветки, либо обновить их из мастера, что бы они не считались устаревшими \n" +
                                "Если по вашей ветке открыта другая задача - нужно исправить имя ветки.\n" +
                                "Указанные ветки находятся в репозитории %s, эти ветки старше %s дней, задачи по ним закрыты:\n",
        ...
        Regressman
                .getInstance()
                .sendPost(
                        Regressman.getInstance().getChannelByName(channelName).getId(),
                        branchMessage,
